...
def ImageProcess(path):
    ...
    # cv2.resizeWindow("Decision", 800, 600)  # Resize the window to 800x600 pixels
    # cv2.waitKey(0)  

    plt.figure(figsize=(10, 6))  # Adjust the size as needed
    plt.imshow(cv2.cvtColor(bottle_clone, cv2.COLOR_BGR2RGB))
    plt.axis('off')  # Hide the axis
    plt.show()

    
for img in glob.glob("C:\\Users\\SwastikAgarwal\\OneDrive - SymphonyAI\\vscodefolder\\tatabottle\Bottle-fill-level-detection--main\\TestImages\\*.jpg"):
    ImageProcess(img)